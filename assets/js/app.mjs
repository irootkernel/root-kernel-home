...
function start() {
  ...
  let floor = playbackFloor(timeline, page);
  ...
  function seekHash() {
    ...
    if(target) {
      position=target.inlineScenes.length ? target.end-.025 : target.answerEnd;
      return true;
    }
    return false;
  }
  if (!restart && seekHash()) restored = true;
  if (!restored) position = floor;
  else floor = restoredPlaybackFloor(timeline,page,position);
  window.addEventListener('hashchange',()=>{
    if (!seekHash()) return;
    frontier=Math.max(frontier,position);
    window.scrollTo(0,scrollOffset(position,unit,floor));
    update(true);
  });
  frontier = Math.max(frontier, position);
  resizeRunway();
  ...
  function resizeRunway() { $('#runway').style.height = `${(timeline.length-floor) * unit + innerHeight}px`; }
  ...
  window.addEventListener('resize', () => {
    ...
    timeline = next;
    floor = restoredPlaybackFloor(timeline,page,position);
    frontier = Math.max(position, frontier, floor);
    unit = scrollUnit();
    resizeRunway();
    lastRendered = '';
    window.scrollTo(0, scrollOffset(position,unit,floor));
  ...
  $('#latest').addEventListener('click', () => window.scrollTo({top: scrollOffset(frontier,unit,floor), behavior: 'instant'}));
  $('#section-select')?.addEventListener('change', event => { location.hash = `step=${encodeURIComponent(event.target.value)}`; });
  document.querySelectorAll('a').forEach(a => a.addEventListener('click', save));

  function navigate(direction) {
    const target = navigatePosition(timeline, position, direction, floor);
    window.scrollTo({top: scrollOffset(target,unit,floor), behavior:motionQuery.matches ? 'instant' : 'smooth'});
  }

  function renderGraph() {
    const fsm = $('#fsm');
    if (!fsm) return;
    const graph = scenario.displayGraph || scenario.graph;
  ...
  function update(immediate = false) {
    ...
    const target = Math.max(floor, Math.min(timeline.length - 0.0001, scrollPosition(scrollY, unit, floor)));
    ...
    frameTime = now;
    if ($('#scroll-action')) $('#scroll-action').textContent = t(position <= floor + 1e-9 ? 'scrollStart' : 'scrollExplore');
    ...
    const progress = (position-floor) / (timeline.length-floor);
    ...
    pairs.forEach((pair, index) => {
      ...
      pair.media.hidden = !mobile || !entry.inlineScenes.length || !p.mediaVisible;
      pair.media.inert = pair.media.hidden;
      pair.media.setAttribute('aria-hidden', String(pair.media.hidden));
      if (mobile && entry.inlineScenes.length && p.mediaVisible) {
        const available = index < point.index ? entry.inlineScenes.length : point.sceneIndex+1;
        const key = `${available}:mobile`;
        if (pair.media.dataset.rendered !== key) {
          const finalFrame = entry.frames.at(-1);
          pair.media.innerHTML = entry.inlineScenes.slice(0,available).map(s=>`<div class="inline-scene" data-scene-id="${s.id}" data-visit-id="${finalFrame.visitId}">${renderPanel(s.id,finalFrame.after,3,locale)}</div>`).join('');
    ...
    $('#previous').disabled = position <= floor + 0.01;
    const pending = snapshot.branches.filter(b => b.required && b.status==='running').length;
    const fsmStatus = $('#fsm-status');
    if (fsmStatus) fsmStatus.textContent = pending ? t('branchesRunning',{n:pending}) : snapshot.branches.length ? t('branchesComplete',{n:snapshot.branches.filter(b=>b.required).length}) : 'STORY MAP';
    const visualGraph = $('#fsm') ? scenario.displayGraph || scenario.graph : null;
    ...
    if ($('#section-select')) $('#section-select').value = point.turn.id;
