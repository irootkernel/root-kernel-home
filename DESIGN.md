...
8. **Page variation.** One machine everywhere (R13 `flight`): launcher pads on the
   ...
   serpentine graph. At 767px and below a static six-node drawing of the same
   procedure replaces the live board: procedure, approval, execute, verify,
   ask-human, closeout, with the declared rework path. Fan-out and fan-in still
   use side rails on the live board. Pending procedure nodes stay legible at rest
   (ash3 outline, iron2 labels); progress reads by colour, never by opacity.
   Compact layouts stack the board or the drawing under the copy. Resizing
   rewrites only presentation geometry: the same 22 nodes, transitions, attempt,
   and 360-tick runtime state remain authoritative on the live board.
